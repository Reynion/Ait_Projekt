...
export default function AdminBoardDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: postData } = await supabase
        ...
        .eq('id', id)
        .single()
      if (!postData) { router.push('/admin/board'); return }
      setPost(postData as unknown as BoardPost)
  ...
  async function handleDeleteComment(commentId: number) {
    ...
    const supabase = createClient()
    await supabase.from('board_comments').update({ deleted_at: new Date().toISOString() }).eq('id', commentId)
  ...
  async function handleDeletePost() {
    ...
    const supabase = createClient()
    await supabase.from('board_posts').update({ deleted_at: new Date().toISOString() }).eq('id', post.id)
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        <Link href="/admin/board" className="text-zinc-500 hover:text-zinc-200 transition-colors text-sm">← 게시글 관리</Link>
      </div>
