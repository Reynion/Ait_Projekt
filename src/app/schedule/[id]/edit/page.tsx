...
export default function ScheduleEditPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (!schedule) { router.push('/schedule'); return }
      if (schedule.created_by !== data.user.id && !admin) { router.push('/schedule'); return }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else { alert('수정에 실패했습니다.'); setSubmitting(false) }
  }
    ...
    <main className="flex min-h-screen flex-col bg-zinc-950">
      ...
      <section className="max-w-lg w-full mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-white">일정 수정</h1>
        </div>
