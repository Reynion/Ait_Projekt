...
export async function GET() {
  ...
  const results = await Promise.all((jobs ?? []).map(async job => {
    const { data: files } = await admin.storage.from('youtube-audio').list(job.job_id)
    const audioFile = files?.find(f => f.name.startsWith('audio.'))
    if (!audioFile) return null

    const { data: pub } = admin.storage.from('youtube-audio').getPublicUrl(`${job.job_id}/${audioFile.name}`)
