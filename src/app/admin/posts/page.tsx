...
function AdminPostsContent() {
  ...
  const [sort, setSort] = useState<'newest' | 'likes'>(() => searchParams.get('sort') === 'likes' ? 'likes' : 'newest')
  ...
  function handleSeasonTabChange(val: number | 'all' | 'none') {
    setSeasonTab(val)
    updateUrl({ season: String(val) })
  }

  function handleSortChange(val: 'newest' | 'likes') {
    setSort(val)
    updateUrl({ sort: val })
  }
  ...
  const filtered = useMemo(() => {
    let list = [...posts]
    if (seasonTab === 'none') list = list.filter(p => p.season_id === null)
    else if (seasonTab !== 'all') list = list.filter(p => p.season_id === seasonTab)
    if (appliedSearch) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <button onClick={() => handleSeasonTabChange('all')} className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${seasonTab === 'all' ? 'bg-zinc-100 text-zinc-900 border-zinc-100' : 'border-zinc-600 text-zinc-400 hover:border-zinc-400'}`}>전체</button>
        {seasons.map(s => (
          <button key={s.id} onClick={() => handleSeasonTabChange(s.id)} className={`text-sm px-3 py-1.5 rounded-full border transition-colors flex items-center gap-1.5 ${seasonTab === s.id ? 'bg-zinc-100 text-zinc-900 border-zinc-100' : 'border-zinc-600 text-zinc-400 hover:border-zinc-400'}`}>
            {s.is_active && <span className="w-1.5 h-1.5 rounded-full bg-green-400 flex-shrink-0" />}
            {s.name}
          </button>
        ))}
        <button onClick={() => handleSeasonTabChange('none')} className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${seasonTab === 'none' ? 'bg-zinc-100 text-zinc-900 border-zinc-100' : 'border-zinc-600 text-zinc-400 hover:border-zinc-400'}`}>미분류</button>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <select value={sort} onChange={e => handleSortChange(e.target.value as 'newest' | 'likes')} className="bg-zinc-800 border border-zinc-600 rounded-lg px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-zinc-400 flex-shrink-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 px-4 py-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {post.seasons && <span className="text-xs text-green-400 bg-green-500/10 border border-green-500/30 px-2 py-0.5 rounded-full flex-shrink-0">{post.seasons.name}</span>}
                </div>
                {post.artist && <p className="text-zinc-500 text-xs truncate">{post.artist}</p>}
              ...
              <div className="flex gap-2 flex-shrink-0">
                <Link href={`/admin/posts/${post.id}`} className="text-xs px-3 py-1.5 rounded-lg border border-zinc-600 hover:border-zinc-400 text-zinc-400 hover:text-white transition-colors">보기</Link>
                <button onClick={() => toggleComments(post.id)} className="text-xs px-3 py-1.5 rounded-lg border border-zinc-600 hover:border-zinc-400 text-zinc-400 hover:text-white transition-colors">💬 댓글 {expanded === post.id ? '닫기' : '보기'}</button>
              ...
              <div className="border-t border-zinc-700 bg-zinc-900 px-4 py-3 flex flex-col gap-2">
                ...
                {(comments[post.id] ?? []).map(comment => (
                  <div key={comment.id} className="flex items-start justify-between gap-3 text-sm">
                    ...
                    <button onClick={() => handleDeleteComment(post.id, comment.id)} className="text-xs text-zinc-500 hover:text-red-400 transition-colors flex-shrink-0">삭제</button>
                  </div>
                ))}
              </div>
            )}
