...
export default function BoardCommentSection({ boardPostId, currentUserId, postAuthorId, link }: Props) {
  ...
  useEffect(() => {
    getCommentPermission('board').then(setCanComment)
  }, [])
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`board-comments-${boardPostId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'board_comments' }, fetchComments)
      .subscribe()
  ...
  async function handleDelete(commentId: number) {
    const supabase = createClient()
    const { error } = await supabase.from('board_comments').update({ deleted_at: new Date().toISOString() }).eq('id', commentId)
